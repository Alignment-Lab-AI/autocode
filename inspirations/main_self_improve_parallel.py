...
def main(process_inputs, process_folder):
    ...
    if not self_improve:
        ...
        file_number = save_generated_code(response, process_folder)
        ...
        for attempt in range(1, num_improvements + 1):
            ...
            save_generated_code(response, process_folder=process_folder, filename=update_filename)
        ...
        for attempt in range(1, num_improvements + 1):
            ...
            response = ask_gpt3(gpt3_question)
            update_filename = f"{process_folder}/{filename_prefix}_update_{attempt}.py"
            save_generated_code(response, process_folder=process_folder, filename=update_filename, use_original_filename=self_improve)
            print(f"\nAssistant: The improved code has been saved as '{update_filename}'.")
...
def run_process(process_number, process_inputs):
    ...
    process_folder = create_process_folder(process_number)
    main(process_inputs, process_folder)
...
def main_concurrent():
    ...
    while True:
        ...
        for i in range(1, num_processes + 1):
            process_inputs = get_process_inputs(i)
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            results = list(executor.map(run_process, process_numbers, process_inputs_list))
